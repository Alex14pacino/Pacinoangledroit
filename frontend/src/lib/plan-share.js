...
import { modeOf, fmtSec, isBw, isPerSide, sideReps, activeRoutines, activePlan } from './history.js'
...
/** Build the shareable bundle: the active plan's entraînements + referenced customs. */
export function buildPlanBundle(S, name) {
  const routines = activeRoutines(S).map(r => ({
    id: r.id, name: r.name, emoji: r.emoji, ...(r.prog ? { prog: r.prog } : {}), ...(r.tags?.length ? { tags: r.tags } : {}), ex: (r.ex || []).map(cleanEx)
  }))
  ...
  return { opengym_plan: PLAN_FMT, exported: todayISO(), name: name || '', routines, customEx }
}
...
export function parsePlan(raw) {
  ...
  if (!data || !data.opengym_plan || !Array.isArray(data.routines)) {
    throw new Error(t('this isn’t a PacinAngle plan file'))
  }
...
export function mergePlan(s, bundle) {
  ...
  // Imported entraînements land in the active plan; if there is none, create one so the
  // import has a home (and something to train right away).
  let target = activePlan(s)
  if (!target) {
    const now = Date.now()
    target = { id: uid(), name: t('My plan'), status: 'active', createdAt: now, startedAt: now, endedAt: null, routines: [] }
    s.plans.push(target)
  }
...
export function planPrintHTML(S, owner) {
  ...
  const routines = activeRoutines(S).filter(r => r.ex && r.ex.length)
...
<body><div class="doc">
  <header>
    <div class="kicker">PacinAngle</div>
    <h1>${esc(t('Weekly Training Plan'))}</h1>
    ${sub ? `<div class="sub">${sub}</div>` : ''}
  </header>
  <h3 class="block">${esc(t('Routines'))}</h3>
  ${body}
  <footer>${esc(t('Made with PacinAngle'))}</footer>
</div></body></html>`
}
