...
// ---- Plans ---------------------------------------------------------------------
// A plan owns its entraînements (the array kept internally as `routines`) and has a
// lifecycle: draft → active → archived. Exactly one plan is active at a time; the app
// trains from it, and each séance is stamped with the active plan's id so stats scope to it.
// (Future DB tables: plans / entrainements / seances — see the model notes.)
export const activePlan = S => (S.plans || []).find(p => p.status === 'active') || null
export const activeRoutines = S => activePlan(S)?.routines || []
export const allRoutines = S => (S.plans || []).flatMap(p => p.routines || [])
export const findRoutine = (S, id) => allRoutines(S).find(r => r.id === id) || null
export const planOfRoutine = (S, id) => (S.plans || []).find(p => (p.routines || []).some(r => r.id === id)) || null

// The rotation: entraînements are trained in list order within the ACTIVE plan, looping.
// "Next" is whatever follows the most recently completed one; with no history yet (or the
// last one deleted), the first in the list. No calendar day is involved.
// Skipping ahead is honoured: finish #3 out of order and #4 comes next, not #2.
export function nextRoutineId(S) {
  const rts = activeRoutines(S)
  if (!rts.length) return null
  for (let i = S.workouts.length - 1; i >= 0; i--) {
    const idx = rts.findIndex(r => r.id === S.workouts[i].routineId)
    if (idx !== -1) return rts[(idx + 1) % rts.length].id
  }
  return rts[0].id
}
export function nextRoutine(S) {
  const id = nextRoutineId(S)
  return id ? activeRoutines(S).find(r => r.id === id) || null : null
}
