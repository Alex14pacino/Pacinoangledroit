...
export function buildDemoState() {
  ...
  for (let d = new Date(start); d <= today; d.setDate(d.getDate() + 1)) {
    ...
    const w = {
      ...
      routineId: routine.id, planId, name: routine.name, bw,
      entries,
  ...
  return {
    plans: [{ id: planId, name: 'Plan 1', status: 'active', createdAt: +start, startedAt: +start, endedAt: null, routines: [push, pull, legs] }],
