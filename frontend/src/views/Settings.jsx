  ...
  const doImport = ev => {
    ...
    rd.onload = () => {
      try {
        ...
        if (!data.workouts || (!data.plans && !data.routines)) throw new Error('not a PacinAngle backup')
  ...
  return <div className="narrow">
    ...
    <Section title={MOBILE ? t('Your data') : DEMO ? t('Demo') : t('Account')}>
      {MOBILE ? <>
        ...
        <Row icon="personCircle" iconTint="var(--grey)" title={user.name} subtitle={t('Signed in — your data syncs to this account.')} />
        ...
        <Row icon="person" iconTint="var(--blue)" title={t('Sign in or create an account')} subtitle={t('Sync your data across your devices.')} accessory="chevron" onClick={() => setGuest(false)} />
      )}
    </Section>
    {!user && !DEMO && !MOBILE && <p className="sect-f" style={{ marginTop: -18, marginBottom: 22 }}>{t('Guest mode — data lives only in this browser.')}</p>}

    {/* ---------- during a workout (rest time is per-exercise now, not a global setting) ---------- */}
    ...
    {!MOBILE && <Section title={t('Tip')}>
      <Row icon="lightbulb" iconTint="var(--yellow)"
        ...
        subtitle={t('to install PacinAngle as a full-screen app.') + ' ' + (user ? t('Your data syncs with your profile — sign in anywhere to see it.') : t('Guest data stays on this device — export a backup now and then!'))} />
    </Section>}

    <div className="dim small" style={{ textAlign: 'center', marginTop: 4, lineHeight: 1.6 }}>
      PacinAngle · {t('free & open source (AGPL v3)')}<br />
...
function NotificationsCard() {
  // Server-sent push needs a backend; the Firebase build has none, so there is no push card.
  return null
}
