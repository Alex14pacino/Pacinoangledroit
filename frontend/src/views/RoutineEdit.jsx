...
export default function RoutineEdit() {
  ...
  const r = findRoutine(S, id)
  const planId = planOfRoutine(S, id)?.id
  const back = planId ? '/plan/p/' + planId : '/plan'
  ...
  return <div className="narrow">
    <div className="hdr">
      ...
      <div style={{ flex: 1, margin: '0 12px' }}>
        <input className="input" defaultValue={r.name} style={{ fontWeight: 600, fontSize: 20, letterSpacing: '-.021em' }}
          onChange={e => update(s => { findRoutine(s, id).name = e.target.value.trim() || t('Routine') })} />
      </div>
    ...
    <Button variant="danger" onClick={() => confirmSheet({
      ...
      onConfirm: () => {
        update(s => {
          const p = s.plans.find(pl => (pl.routines || []).some(x => x.id === id))
          if (p) p.routines = p.routines.filter(x => x.id !== id)
        })
        nav(back)
      }
