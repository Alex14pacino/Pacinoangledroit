...
import { fmtDate, fmtNum, fmtVol, fmtDur, durPart, todayISO, uid, exCount, MONTHS_LONG, ACCENTS } from './lib/format.js'
import { lastEntryFor, bestWeightFor, buildSets, workoutVolume, setsDone, setsDoneActive, lastBW, supersetUnits, unitOf, setLabel, defaultConfig, cleanupSg, modeOf, effortOf, isBw, isPerSide, sideReps, activePlan, activeRoutines, findRoutine } from './lib/history.js'
...
export function loadStarterPlan() {
  ...
  update(st => {
    // Land in an active plan straight away so a new user can train immediately.
    let p = activePlan(st)
    if (!p) {
      const now = Date.now()
      p = { id: uid(), name: t('My plan'), status: 'active', createdAt: now, startedAt: now, endedAt: null, routines: [] }
      st.plans.push(p)
    }
    p.routines.push(push, pull, legs)
  })
  toast(t('Starter plan loaded — Push · Pull · Legs'))
}
...
function AddToRoutine({ ex, close }) {
  ...
  const routines = ap?.routines || []
  const pick = rid => {
    close()
    if (!ap) { toast(t('Activate a plan first')); nav('/plan'); return }
    const isNew = rid === '_new'
    exConfigSheet(ex, null, cfg => {
      let newId = null
      update(s => {
        const p = s.plans.find(x => x.id === ap.id)
        const r = isNew ? { id: uid(), name: t('New routine'), emoji: DEFAULT_GLYPH, ex: [] } : p.routines.find(x => x.id === rid)
        if (isNew) { p.routines.push(r); newId = r.id }
        if (r) r.ex.push({ id: ex.id, ...cfg })
      })
      const r = isNew ? findRoutine(S(), newId) : routines.find(x => x.id === rid)
      toast(t('“{0}” added to {1}', ex.n, r ? r.name : t('routine')))
      if (isNew && r) nav('/plan/r/' + r.id)
    }, null, isNew ? null : routines.find(x => x.id === rid))
  }
  return <>
    ...
    <div className="list">
      {routines.map(r => <div key={r.id} className="item" onClick={() => pick(r.id)}>
  ...
  confirmSheet({
    ...
    onConfirm: () => {
      update(s => {
        ...
        s.plans.forEach(p => p.routines.forEach(r => { r.ex = r.ex.filter(e => e.id !== ex.id); cleanupSg(r.ex) }))
...
function usageMap(st) {
  const u = {}
  st.plans.forEach(p => p.routines.forEach(r => r.ex.forEach(e => { u[e.id] = (u[e.id] || 0) + 1 })))
  st.workouts.forEach(w => w.entries.forEach(e => { u[e.id] = (u[e.id] || 0) + 1 }))
  return u
}
function ExercisePicker({ onPick, close, tags }) {
  const st = useStore(s => s.S)
  ...
  // A tagged workout restricts the picker to exercises from its body parts.
  const tagged = tags && tags.length ? tags.filter(b => BODYPARTS.includes(b)) : null
  const all = tagged ? allExercises(st).filter(e => tagged.includes(e.bp)) : allExercises(st)
  const bpList = tagged || BODYPARTS
  ...
  return <>
    ...
    {tagged && <div className="muted small" style={{ marginTop: -4, marginBottom: 8 }}>{t('Limited to this workout’s tags:')} {tagged.map(b => t(b)).join(', ')}</div>}
    ...
    <div className="chips" style={{ margin: eqOpts.length > 1 ? '10px 0 6px' : '10px 0' }}>
      ...
      {bpList.map(b => <button key={b} className={'chip' + (bp === b ? ' on' : '')} onClick={() => { setBp(b); setEq(''); setShown(50) }}>{t(b)}</button>)}
    </div>
...
export const exercisePicker = (onPick, tags) => ui().openSheet(close => <ExercisePicker onPick={onPick} close={close} tags={tags} />)
...
function ExConfig({ ex, existing, onSave, onDelete, close, routine }) {
  ...
  const save = () => {
    ...
    if (cardio) onSave({ sets, min: Math.max(1, Math.round(c.min) || 20), speed: Math.max(0, c.speed || 8), ...meta })
      ...
      const out = { sets, mode: 'reps', reps, weight: Math.max(0, c.weight || 0), ...flags, ...(perSide ? { side: true } : {}), ...prog, ...meta }
...
function PlanTools({ close }) {
  ...
  const hasRoutines = activeRoutines(st).some(r => r.ex && r.ex.length)
  ...
  return <>
    ...
    <div className="dim small" style={{ margin: '7px 2px 0', lineHeight: 1.4 }}>{t('A small file a friend imports into their own PacinAngle — routines only, none of your workouts or weigh-ins.')}</div>
    {!MOBILE && <>
      <div style={{ height: 12 }} />
...
function Calendar({ start, close }) {
  ...
  return <>
    ...
    <div className="small dim" style={{ textAlign: 'center', marginTop: 10 }}>{t('Tap a trained day for details')}</div>
  </>
}
...
export function WorkoutRow({ w, onClick }) {
  const st = useStore(s => s.S)
  const glyph = glyphOf((findRoutine(st, w.routineId) || {}).emoji)
...
export function startFlow(routineId) {
  // No pre-workout weigh-in — start straight away. Body weight is still logged any time
  // from the Home / Stats "Log" button.
...
function doFinishWorkout() {
  ...
  const w = {
    id: A.id, d: A.d, start: A.start, end: Date.now(), routineId: A.routineId, planId: A.planId || null, name: A.name, bw: A.bw,
